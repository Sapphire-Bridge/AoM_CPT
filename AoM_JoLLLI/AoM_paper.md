...
3. Constraint on simplistic locality (SDH stress test): a fixed-depth target-specificity analysis that yields a local matched-span contrast at one tested layer, showing target-span concentration over nearby matched controls in the checked-in artifact for all non-GPT‑2 models while remaining silent on global token‑atomic exclusivity and full semantic specificity.
...
Under the canonical matched-token nearby-span control used in the checked-in SDH artifact, target-minus-control signed deltas are positive for all non-GPT‑2 models and negative for GPT‑2 at fixed quarter-depth. We therefore interpret SDH as evidence of target-span concentration relative to nearby matched spans under this local protocol, not as full semantic specificity. [E5a, E5b, E5c, E5d, E5e, E5f, C4]
The checked-in SDH artifact is protocol-conditional: it is evaluated at fixed (ℓ*) within a bounded local window and one matched-token control path, not at each model’s sweep-selected maximizing layer and not over global position space. Because the canonical artifact does not vary control-selection strategy within Table 3, we do not draw strategy-sensitivity conclusions from that table. [C4, E5a, E5b, E5c, E5d, E5e, E5f]
...
The SDH stress test adds a second constraint: at fixed depth (ℓ*) and within a local position window (position_window=8, buffers ±2), nearby matched-span controls still produce non-zero donor-directed effects, but in the checked-in canonical artifact target-minus-control signed deltas are positive for all non-GPT‑2 models and negative for GPT‑2. The appropriate mechanistic reading is target-span concentration under a local matched-span control, not full semantic specificity or token-atomic exclusivity. [C4, E5a, E5b, E5c, E5d, E5e, E5f]
...
The fixed-depth target-specificity analysis adds a local constraint, but under the canonical matched-token nearby-span control the checked-in artifact is better read as target-span concentration than as local non-specificity: target-minus-control signed deltas are positive for all non-GPT‑2 models and negative for GPT‑2 at the tested depth. This does not show that the target span is always privileged, and it does not amount to full semantic specificity; the result is a local matched-span contrast at one fixed layer, and specificity can vary by depth or control design. Nor does it discriminate Kaplan versus Braun as semantic theories. What it does show is that nearby matched spans can retain non-zero causal leverage even when target-span patching is usually stronger in the canonical artifact.

What SDH rules out.
At the tested depth (ℓ*) and within the specified local window, the checked-in matched-token control does not support a claim of target-span exclusivity or a claim of equal leverage across nearby spans. GPT‑2 is negative, several intervals are wide, and control effects remain non-zero. The narrower conclusion is that target-span concentration can coexist with distributed causal relevance in nearby matched spans.

What SDH is consistent with.
The observed pattern is consistent with an implementation in which target-span interventions often have larger leverage than nearby matched-span controls while those nearby positions still provide partial causal access to the same downstream preference margin. This supports a mixed picture: local concentration without token-atomic exclusivity.
...
The AoM program is intended to change what must be argued, not to settle metaphysics. The results in this paper provide (i) behavioral evidence for a consistent (suite-conditional) pattern of context-sensitivity across DISAMB/CF/COH under controlled manipulations, and (ii) causal evidence that internal token-in-context states at specific depths exert donor-directed control over AoM-relevant preference margins under activation patching, with sham baselines near zero and structured depth profiles. SDH adds a further mechanistic constraint: at the fixed tested depth and within a local neighborhood, nearby matched-span controls remain causally active, but the checked-in canonical artifact usually shows stronger target-span effects for the non-GPT‑2 models. These findings do not establish meaning proper, but they do make purely dismissive “mere surface” moves less informative: to maintain a meaning-beyond-AoM position while conceding AoM-like competence, one must identify an additional property that is (1) competence-relevant, (2) plausibly absent here, and (3) associated with detectable signatures that could in principle be measured.
...
Table 3. SDH fixed-depth target-minus-nearby-matched-span results at ℓ* = round(0.25 × (L−1)) under the canonical `matched_token` control, with `position_window=8` and exclusion buffers ±2. Reported effects are signed donor-directed margin shifts; Δ = E_target − E_ctrl and win rate = P(Δ_i>0). N directions per model = 104. This checked-in artifact does not vary control-selection strategy within the table. [E5a, E5b, E5c, E5d, E5e, E5f, C4]
